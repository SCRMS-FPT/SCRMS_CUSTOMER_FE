...
const FeaturedVenues = () => {
  ...
  const handleBookNow = (sportCenterId) => {
    navigate(`/booking/${sportCenterId}`);
  };
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-10">
          ...
          <p className="text-lg text-gray-600 max-w-3xl mx-auto">
            Discover top-rated sports facilities in your area. Filter by sport type and book your next game with ease.
          </p>
        </div>

        {/* Search and filter section */}
        <div className="mb-8">
          <form onSubmit={handleSearch} className="flex flex-col md:flex-row gap-3 p-3 bg-white rounded-xl shadow-sm border border-gray-100">
        ...
        <div className="mb-6">
          <p className="text-gray-600">
            Showing <span className="font-medium">{filteredVenues.length}</span> venues
            {selectedSport !== "All" && (
              <span> for <span className="font-medium">{selectedSport}</span></span>
            )}
            {searchTerm && (
              <span> matching "<span className="font-medium">{searchTerm}</span>"</span>
            )}
            {searchLocation && (
              <span> in <span className="font-medium">{searchLocation}</span></span>
            )}
          </p>
        </div>
                ...
                <div className="bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-shadow border border-gray-100 h-full flex flex-col">
                  <div className="relative">
                    ...
                    <div className="h-48 relative overflow-hidden">
                      <img
                        src={center.avatar || 'https://placehold.co/600x400?text=Sport+Center'}
                        alt={center.name}
                    ...
                    <div className="absolute top-4 left-4 flex flex-wrap gap-1">
                      {center.sportNames && center.sportNames.slice(0, 3).map((sport, idx) => (
                        <span key={idx} className="px-3 py-1 bg-blue-600 text-white text-xs rounded-full font-medium">
                          {sport}
                        </span>
                      ))}
                  ...
                  <div className="p-4 flex-grow flex flex-col">
                    ...
                    <p className="text-sm text-gray-600 mb-3 flex-grow line-clamp-3">
                      {center.description || `Experience premium sports facilities at ${center.name}. Book now for an outstanding sports experience.`}
                    </p>
