...
const CourtCard = ({ court }) => {
  ...
  const formatDateRange = (dateRange) => {
    ...
    return `${new Date(dateRange.start).toLocaleDateString(
      "en-US",
      options
    )} - ${new Date(dateRange.end).toLocaleDateString("en-US", options)}`;
  };
  ...
  const isUnavailable =
    ...
    court.dateRange.start > today ||
    court.dateRange.end < today;
  ...
  return (
    <div
      className={`bg-white border border-gray-300 rounded-lg shadow-md overflow-hidden mb-4 flex transition-all hover:shadow-lg ${
    ...
    >
      ...
      <div className="p-4 flex-grow flex flex-col justify-between">
        <div>
          ...
          {isUnavailable ? (
            <p className="text-red-600 font-bold bg-red-100 px-2 py-1 rounded-md w-max">
              🚫 Unavailable
            </p>
          ) : (
            <p className="text-blue-600 font-medium">
              📅 {formatDateRange(court.dateRange)}
            </p>
          )}
        </div>
            ...
            <div className="mt-2 flex flex-wrap gap-2">
              {court.sport.map((sport, index) => {
                ...
                return (
                  ...
                  >
                    {icon && <img src={icon} alt={sport} className="w-4 h-4" />}{" "}
                    {sport}
                  </span>
                );
              })}
