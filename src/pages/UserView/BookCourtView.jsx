...
const BookCourtView = () => {
    ...
    try {
      ...
      navigate('/booking-success', {
        state: {
          ...
          totalMinutes: Object.values(selectedTimeSlots).reduce((total, slots) =>
            total + slots.length * 30, 0),
  ...
  const getCourtStatusBadge = (status) => {
    switch (status) {
      case 0: return <Badge status="success" text="Available" />;
      case 1: return <Badge status="warning" text="Busy" />;
      case 2: return <Badge status="error" text="Maintenance" />;
                  ...
                  <Row gutter={[16, 16]}>
                    {courts.map((court) => (
                      <Col key={court.id} xs={12} sm={8} md={6}>
                        <Card
                          hoverable
                          className={selectedCourtId === court.id ? 'selected-court' : ''}
                          onClick={() => handleCourtSelect(court.id)}
                          style={{
                            textAlign: 'center',
                            border: selectedCourtId === court.id ? '2px solid #1890ff' : '1px solid #f0f0f0',
                            background: selectedCourtId === court.id ? '#e6f7ff' : 'white'
                          }}
                  ...
                  <Alert
                    message={
                      <span>
                        ...
                        Selected Date: <Text strong>{formatDate(selectedDate)}</Text>
                      </span>
                    }
                    type="info"
                    showIcon={false}
                  ...
                  <Row gutter={[8, 8]}>
                    {getAvailableSlotsForSelectedCourt().map((slot, index) => (
                      <Col key={index} xs={12} sm={8} md={6}>
                        <Button
                          type={selectedTimeSlots[slot.courtId]?.some(s => s.time === slot.time) ? 'primary' : 'default'}
                          disabled={!slot.isAvailable}
                          onClick={() => toggleTimeSlot(slot)}
                          style={{ width: '100%' }}
                          className={!slot.isAvailable ? 'time-slot-unavailable' : 'time-slot'}
                        >
                          {slot.displayTime}
                        </Button>
                      </Col>
              ...
              >
                ...
                >
                  ...
                  <Descriptions.Item label="Your Information" span={2}>
                    <div>
                      <div>{form.getFieldValue('firstName')} {form.getFieldValue('lastName')}</div>
                ...
                <List
                  header={<div style={{ fontWeight: 'bold' }}>Selected Time Slots</div>}
                  bordered
                      ...
                      >
                        <List.Item.Meta
                          ...
                          description={
                            <div>
                              {slots.map((slot, index) => (
                                <Tag key={index} color="blue">{slot.displayTime}</Tag>
                              ))}
                ...
                <Radio.Group defaultValue="venue" style={{ width: '100%', marginBottom: 24 }}>
                  <Space direction="vertical" style={{ width: '100%' }}>
                    <Radio value="venue">
                      ...
                      >
                        <div style={{ display: 'flex', alignItems: 'center' }}>
                          <BankOutlined style={{ fontSize: 20, marginRight: 12, color: '#1890ff' }} />
                          <div>
                    ...
                    <Radio value="online">
                      ...
                      >
                        <div style={{ display: 'flex', alignItems: 'center' }}>
                          <CreditCardOutlined style={{ fontSize: 20, marginRight: 12, color: '#52c41a' }} />
                          <div>
                ...
                <div style={{ marginTop: 24, display: 'flex', justifyContent: 'space-between' }}>
                  ...
                  <Button
                    ...
                    onClick={() => {
                      ...
                      const description = `Booking ${selectedCourt?.courtName} on ${formatDate(selectedDate)} for ${Object.values(selectedTimeSlots).reduce((total, slots) => total + slots.length * 30, 0)} minutes`;
                ...
                <>
                  ...
                  <List
                    ...
                    dataSource={[
                      {
                        title: 'Court Type',
                        description: formatCourtType(getSelectedCourt().courtType),
                  ...
                  {getSelectedCourt().facilities && getSelectedCourt().facilities.length > 0 && (
                    <>
              ...
              >
                <List
                  ...
                  dataSource={[
                    ...
                    {
                      title: 'Courts',
                      description: `${Object.keys(selectedTimeSlots).length} selected`,
                      icon: <AppstoreOutlined style={{ color: '#1890ff' }} />
                    },
                    {
                      title: 'Total Duration',
                      description: `${Object.values(selectedTimeSlots).reduce((total, slots) =>
                        total + slots.length * 30, 0)} minutes`,
